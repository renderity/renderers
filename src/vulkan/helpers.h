...
namespace RDTY::VULKAN::HELPERS
{
	...
	struct Device
	{
		...
		uint64_t                         transfer_queue_count        = 0;
		uint32_t                         compute_queue_family_index  = -1;
		uint64_t                         compute_queue_count         = 0;
		...
		void getProps (VkPhysicalDevice physical_device, VkSurfaceKHR surface)
		{
			...
			for (uint64_t i = 0; i < queue_family_prop_count; ++i)
			{
				...
				// const bool transfer { queue_family_props[i].queueFlags & VK_QUEUE_TRANSFER_BIT || 0 };
				// const bool compute { queue_family_props[i].queueFlags & VK_QUEUE_COMPUTE_BIT || 0 };
				// const bool graphics { queue_family_props[i].queueFlags & VK_QUEUE_GRAPHICS_BIT || 0 };
				// const bool present { surface_support };
				...
				{
					cout << "ASDAS" << endl;
					cout << (queue_family_props[i].queueFlags & VK_QUEUE_GRAPHICS_BIT || 0) << endl;
					cout << !(queue_family_props[i].queueFlags & VK_QUEUE_GRAPHICS_BIT || 0) << endl;
					cout << !(1) << endl;
					cout << (123 || 0) << endl;
		...
		VkImageView ImageView (const VkImageViewCreateInfo&& info, const VkAllocationCallbacks* pAllocator = nullptr)
		{
			VkImageView image_view = VK_NULL_HANDLE;
